...
export default function AboutMe() {
  ...
  const colors = {
    primary: 'text-teal-500',         // Primary accent color
    primaryBg: 'bg-teal-500',         // Primary background
    secondary: 'text-teal-400',       // Secondary accent
    secondaryBg: 'bg-teal-600',       // Secondary background
    dark: 'text-gray-800',            // Dark text
    darkBg: 'bg-teal-800',            // Dark background
    light: 'text-white',              // Light text
    lightBg: 'bg-teal-600',           // Light background
    cardBg: 'bg-white',               // Card background
    overlay: 'bg-teal-900/20',        // Image overlay
    buttonBg: 'bg-teal-600',          // Button background
    buttonHover: 'hover:bg-teal-700'  // Button hover
  }
          ...
          <div className="grid grid-cols-3 gap-4 mt-8">
            {stats.map((stat, index) => (
              <motion.div
                key={index}
                whileHover={{ y: -5 }}
                className={`${colors.cardBg} p-4 rounded-lg text-center shadow-lg border border-teal-100`}
              >
                <div className={`${colors.primary} text-2xl mb-2 flex justify-center`}>
                  {stat.icon}
                </div>
                <h3 className={`text-2xl font-bold ${colors.dark}`}>{stat.value}</h3>
                <p className={`${colors.primary} font-medium`}>{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="relative">
          <div className="relative aspect-square max-w-md mx-auto">
            ...
            <motion.div
              ...
              className={`absolute -bottom-6 -left-6 ${colors.primaryBg} p-4 rounded-lg shadow-lg z-20 max-w-xs`}
            >
              <div className="flex items-center gap-3">
                <div className={`p-2 ${colors.cardBg} rounded-full`}>
                  <FiCode className={`text-xl ${colors.primary}`} />
                </div>
                <div>
            ...
            <motion.div
              ...
              className={`absolute -top-6 -right-6 ${colors.secondaryBg} p-4 rounded-lg shadow-lg z-20 max-w-xs`}
            >
              <div className="flex items-center gap-3">
                <div className={`p-2 ${colors.cardBg} rounded-full`}>
                  <FiUser className={`text-xl ${colors.secondary}`} />
                </div>
                <div>
